...
def tower_loss( scope, dataset ):
	...
	for l in losses + [total_loss]:
		...
		loss_name = re.sub('%s_[0-9]*/' % CNN_DecNN.TOWER_NAME, '', l.op.name)
...
def train( dataset ):
	...
	with tf.Graph().as_default(), tf.device('/cpu:0'):
		...
		decay_steps = int(num_batches_per_epoch * CNN_DecNN.NUM_EPOCHS_PER_DECAY)
		...
		for i in xrange(FLAGS.num_gpus):
			with tf.device('/gpu:%d' % i):
				with tf.name_scope('%s_%d' % (CNN_DecNN.TOWER_NAME, i)) as scope:
		...
		variable_averages = tf.train.ExponentialMovingAverage(
				CNN_DecNN.MOVING_AVERAGE_DECAY, global_step)
