...
def main(argv=None):	# pylint: disable=unused-argument
	...
	print(x_data)
	pooled = slim.max_pool2d( x_data, [2,2], stride=2 )
	#pooled, argmax = tf.nn.max_pool_with_argmax( x_data, [1,2,2,1], [1,1,1,1], "VALID" )
	argmax = np.ndarray( shape=(1,3,3,1), dtype=np.int64 )
	argmax.fill(0)
	argmax[0,0,1,0]=2
	argmax[0,0,2,0]=4
	argmax[0,1,0,0]=12
	...
	pdb.set_trace()
	unpooled = unpool_layer_with_argmax_batch( pooled, 2, argmax )
	unpooled_wo_argmax = unpool2x2( pooled )
	#unpooled_wo_argmax = unpool_layer_without_argmax_batch( pooled, 3 )
	#unpooled_wo_argmax = FixedUnPooling( pooled, 2 )
